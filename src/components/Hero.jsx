...
const FALLBACK = {
  ...
  bio: 'Building intelligent systems at the intersection of surgical robotics, medical imaging, and AI.',
  email: 'kethansai.official@gmail.com',
  phone: '+91 9581761578',
  linkedin: 'https://linkedin.com/in/kethansaiv',
  github: 'https://github.com/KethanSaiV',
  resume_url: '#',
  video_url: 'https://rgtatjqxochctgjvhyuq.supabase.co/storage/v1/object/public/vid/int.mp4',
  video_opacity: 0.08,
  photo_size: 220,
};
...
export default function Hero() {
  ...
  const videoOpacity = about.video_opacity ?? FALLBACK.video_opacity;
  const photoSize = about.photo_size ?? FALLBACK.photo_size;
  const videoUrl = about.video_url ?? FALLBACK.video_url;
  ...
  return (
    ...
    >
      ...
      <nav style={{
        ...
        <span style={{ fontFamily: 'var(--font-display)', fontSize: '1.1rem', letterSpacing: '0.15em', color: 'var(--red)' }}>KSV</span>
        <div style={{ display: 'flex', gap: '1.5rem' }}>
          {['Projects', 'Publications', 'Experience', 'Skills', 'Contact'].map(s => (
            <a key={s} href={`#${s.toLowerCase()}`} style={{
              ...
              letterSpacing: '0.12em', color: 'var(--muted)', transition: 'color 0.2s',
            }}
              onMouseEnter={e => e.target.style.color = 'var(--text)'}
              onMouseLeave={e => e.target.style.color = 'var(--muted)'}
            >{s}</a>
          ))}
          <a href="/admin" style={{ fontFamily: 'var(--font-display)', fontSize: '0.78rem', letterSpacing: '0.12em', color: 'var(--muted)' }}>Admin</a>
        </div>
      </nav>
      ...
      <div className="container" style={{ paddingBottom: '5rem', paddingTop: '10rem', position: 'relative', zIndex: 2 }}>
        ...
        <div className="fade-up delay-1" style={{ display: 'flex', alignItems: 'center', gap: '2.5rem', flexWrap: 'wrap' }}>
          <h1 className="display" style={{ fontSize: 'clamp(4rem, 12vw, 11rem)', color: 'var(--text)', maxWidth: '900px', lineHeight: 0.9 }}>
        ...
        <p className="fade-up delay-2" style={{
          fontFamily: 'var(--font-body)', fontSize: 'clamp(1rem, 1.4vw, 1.25rem)',
          color: 'var(--muted)', maxWidth: '560px', marginTop: '1.5rem', lineHeight: 1.6, fontWeight: 300,
        }}>
        ...
        <div className="fade-up delay-4" style={{ display: 'flex', gap: '1.5rem', marginTop: '3rem', flexWrap: 'wrap' }}>
          {[
            ...
            <a key={label} href={url} target="_blank" rel="noreferrer" style={{
              fontFamily: 'var(--font-display)', fontSize: '0.78rem', letterSpacing: '0.15em',
              color: 'var(--muted)', borderBottom: '1px solid transparent', paddingBottom: '2px', transition: 'color 0.2s, border-color 0.2s',
            }}
        ...
        <div style={{ marginTop: '2rem' }}>
          <span style={{
            fontFamily: 'var(--font-display)', fontSize: '0.7rem', letterSpacing: '0.2em',
            color: 'var(--muted)', background: 'var(--surface)', border: '1px solid var(--border)', padding: '0.3rem 0.8rem',
          }}>
            📍 CHENNAI, INDIA · B.TECH AI & MEDICAL ENGINEERING · SRIHER
          </span>
        </div>
      </div>
      ...
      <div style={{
        ...
        fontFamily: 'var(--font-display)', fontSize: 'clamp(12rem, 22vw, 26rem)', lineHeight: 1,
        color: 'transparent', WebkitTextStroke: '1px rgba(255,60,0,0.08)',
